...
from db.typings import DBConnection, QueryComponents
...
async def write_event(event: Event) -> None:
    ...
    async with pool.connection() as conn:
        ...
        p_tags = tuple((event.id, tag.pubkey, tag.recommended_relay_url) for tag in event.tags if type(tag) is P_Tag)
        ...
        await run_queries(no_return_queries=[(e_tag_q, flat_list(e_tags)), (p_tag_q, flat_list(p_tags))], conn=conn)


async def filter_events(*filters: Filters) -> list[Event]:
    ...
    if f_e_tags := flat_list(f.e_tag for f in filters if f.e_tag):
        ...
        e_tag_in_clause = prepare_in_clause((E_TAG_TABLE_NAME, "event_id"), len(f_e_tags))
    ...
    if f_p_tags := flat_list(f.p_tag for f in filters if f.p_tag):
        ...
        p_tag_in_clause = prepare_in_clause((P_TAG_TABLE_NAME, "pubkey"), len(f_p_tags))
    ...
    async with pool.connection() as conn:
        ...
        in_clause = prepare_in_clause((E_TAG_TABLE_NAME, "associated_event"), value_count=1)  # len(event_ids)
        ...
        in_clause = prepare_in_clause((P_TAG_TABLE_NAME, "associated_event"), value_count=1)  # len(event_ids)
    ...
    event_e_tags = dict(groupby(sorted(tags.get("#e", []), key=lambda e: e[0]), lambda e: e[0]))
    event_p_tags = dict(groupby(sorted(tags.get("#p", []), key=lambda p: p[0]), lambda p: p[0]))
    print("@" * 100)
    print(tags)
    print("#" * 100)
    print(event_e_tags)
...
async def fetch_event(event_id: str) -> Event | None:
